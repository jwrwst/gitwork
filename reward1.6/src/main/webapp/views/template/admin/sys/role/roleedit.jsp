...
<div class="pageContent">
	<form method="post" action="${webpath }/rs/inner/sysRoleInfo/save" name="addform"  class="pageForm required-validate" onsubmit="return validateCallback(this,pageRefushChild);">
		<div class="pageFormContent" layoutH="58">
			...
			<div class="unit">
				<label>角色名称：</label>
				<input type="hidden" name="sysRoleId" value="${sysRoleInfoVo.sysRoleId }">
				<input type="text" size="25" name="roleName" value="${sysRoleInfoVo.roleName}"/>
			</div>
			<div class="divider">divider</div>
			          ...
			          if("${volist.parentId}"==0){
				            ...
				            $("#treeul").find("li").each(function(t){
				                if("${volist.parentId}"==$(this).attr("id")){
				                    ...
				                    if("${volist.ischeck}"=="true"){
				                      $("#ul"+$(this).attr("id")).append("<li id='${volist.sysAuthId}'><a  tname='name' tvalue='${volist.sysAuthId}' checked=''>${volist.authName}</a></li>");
				                    }else{
