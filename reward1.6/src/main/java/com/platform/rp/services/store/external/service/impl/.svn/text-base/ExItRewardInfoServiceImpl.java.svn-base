...
@Service
public class ExItRewardInfoServiceImpl implements IExItRewardInfoService {
	
	private Log log = LogFactory.getLog(ExBsEmployeeExtRewardServiceImpl.class);

	@Autowired
	private ItRewardInfoDAO<ItRewardInfoEntity> itRewardInfoDAO;
	...
	public List<StoreRewardStatisticVo> getStoreRewardStatisticByStoreID(String storeID, String filter,String startTime,String endTime) {
		...
		return itRewardInfoDAO.getStoreRewardStatisticByStoreID(params);
	}
...
//			params.put("end", page.getEnd());
			...
			// 查询总数
			int count = itRewardInfoDAO.count(params);
			
			//查询数据
			List<EmployeeRankVo> employeeRankList = itRewardInfoDAO.getPage(params);
	...
	public void deleteByOrderNum(String orderNum)throws Exception{
		try {
			itRewardInfoDAO.deleteByOrderNum(orderNum);
		} catch (Exception e) {
			throw e;
		}
	}
	...
	public ItRewardInfoEntity getRewardInfoByOrderNum(String orderNum)throws Exception{
		try {
			return itRewardInfoDAO.getRewardInfoByOrderNum(orderNum);
		} catch (Exception e) {
			throw e;
		}
	}
	...
	public EmployeeRankDetailVo getEmployeeRewardRankDetail(long storeID, long empID) {
		...
		EmployeeRankDetailVo empRankDetailVo = itRewardInfoDAO.getEmployeeRewardRankDetail(params);
