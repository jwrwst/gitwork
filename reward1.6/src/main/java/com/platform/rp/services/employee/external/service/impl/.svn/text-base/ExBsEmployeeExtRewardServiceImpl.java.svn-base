...
@Service
public class ExBsEmployeeExtRewardServiceImpl implements IExBsEmployeeExtRewardService {
...
//			params.put("end", page.getEnd());
			...
			// 查询总数
			int count = empExtRewardDAO.count(params);
			List<StoreRankVo> storeRankList = empExtRewardDAO.getPage(params);
	...
	public EmployeeRankDetailVo getEmployeeRewardRankDetail(long storeID, long empID) {
		...
		EmployeeRankDetailVo empRankDetailVo = empExtRewardDAO.getEmployeeRewardRankDetail(params);
	...
	public EmployeeIncomeBaseInfoVo getEmployeeIncomeBaseInfo(long empID) {
		return empExtRewardDAO.getEmployeeIncomeBaseInfo(empID);
	}

	@Override
	public void update(BsEmployeeExtRewardEntity vo) {
		empExtRewardDAO.update(vo);
	}
}
